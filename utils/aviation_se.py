...
def display_result(result):
    st.markdown(f"""
    ### ![favicon]({result['favicon']}) [{result['title']}]({result['link']})
    {result['snippet']}
    """
    )
...
def handle_pagination(pagination, n_pages):
    ...
    # assert(int(current) == st.session_state.search_page)
    ...
    #     radio_label = radio_label[max(0, st.session_state.search_page - 4):min(st.session_state.search_page + 4, len(radio_label) - 1)]
    
    if st.session_state.search_page > 1:
        col1.button("prev", on_click=lambda: setattr(st.session_state, 'search_page', st.session_state.search_page - 1))
    if st.session_state.search_page < n_pages:
        col3.button("next", on_click=lambda: setattr(st.session_state, 'search_page', st.session_state.search_page + 1))

    col2.write(f"{(st.session_state.search_page-1) * num + 1} -- {(st.session_state.search_page * num)}")
...
def display_response(response):
    ...
    if not response["organic_results"]:
        return
    if not "total_results" in response["search_information"]:
        return
...
def google_search(search_input, page):
    if not search_input:
        return
